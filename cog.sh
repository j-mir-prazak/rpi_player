...
function looping {
	while true; do
	  ...
	  ./omx.sh &
		PROC2=$!
		trap 'echo loopcogs; kill -SIGTERM $PROC2 2>/dev/null; trap SIGINT; terminate' SIGTERM
		trap 'echo loopcogs; kill -SIGINT $PROC2 2>/dev/null; trap SIGINT; terminate' SIGINT
		wait
		echo ""
